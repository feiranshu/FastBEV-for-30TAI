...
//   The rounding add and variable right shift are separated so the 200 MHz
//   path no longer contains both operations plus saturation in one cycle.
//====================================================================
`timescale 1ns/1ps
...
module fp32_int8_quant #(
    ...
    // Datapath registers intentionally have no asynchronous clear. Only the
    // valid pipeline is reset; data is ignored whenever valid is zero. This
    // removes thousands of reset pins from the core clock domain.
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            s2_shamt   <= s1_shamt;
            s2_rounded <= s1_rounded;
            s2_v       <= s1_v;
        end
    end

    wire [39:0] s2_shifted = s2_rounded >> s2_shamt;
    wire [8:0]  s2_mag_w   = (s2_shifted > 40'd128) ? 9'd128 : s2_shifted[8:0];
    ...
    always @(*) begin
        if (s3_is_zero)
            ...
            cand = s3_sign ? -9'sd128 : 9'sd127;
        else if (s3_sign)
            cand = -$signed({1'b0, s3_mag});
        else
            cand = (s3_mag > 9'd127) ? 9'sd127 : $signed({1'b0, s3_mag});
    end
