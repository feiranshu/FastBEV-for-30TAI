...
//     0x24  EXT_MODE
//     0x25  FEAT2D_FP32_BASE
//     0x26  FEAT2D_INT8_BASE
//     0x27  CONCAT_OUT_BASE
//     0x28  GROUP_STATUS (R)
//     0x29  GROUP_CTRL
//     0x2A  ERROR_STATUS (R/W1C)
//     0x2B  PERF_STAGE_SEL
//     0x2C~0x2D  PERF_STAGE_CNT (R)
//     0x30  VERSION (R)
//     0x40~0x51  XFORM_H0/H1/H2 matrices (Q16.16)
//     0x77  RESET (W)
//     0xFF  DEBUG (R/W)
//==============================================================================
...
module bev_reg_ctrl(    
    ...
    output reg    [    2 : 0]   ext_mode         ,
    output reg    [   31 : 0]   feat2d_fp32_base ,
    output reg    [   31 : 0]   feat2d_int8_base ,
    output reg    [   31 : 0]   concat_out_base  ,
    output reg    [   31 : 0]   group_ctrl       ,
    input         [   31 : 0]   group_status     ,
    input         [   31 : 0]   group_error_status,
    output reg    [   31 : 0]   error_status_w1c ,
    output reg                  error_status_w1c_valid,
    output reg    [   31 : 0]   perf_stage_sel   ,

    output reg    [   31 : 0]   xform_h0_a00     ,
    output reg    [   31 : 0]   xform_h0_a01     ,
    output reg    [   31 : 0]   xform_h0_a02     ,
    output reg    [   31 : 0]   xform_h0_a10     ,
    output reg    [   31 : 0]   xform_h0_a11     ,
    output reg    [   31 : 0]   xform_h0_a12     ,
    output reg    [   31 : 0]   xform_h1_a00     ,
    output reg    [   31 : 0]   xform_h1_a01     ,
    output reg    [   31 : 0]   xform_h1_a02     ,
    output reg    [   31 : 0]   xform_h1_a10     ,
    output reg    [   31 : 0]   xform_h1_a11     ,
    output reg    [   31 : 0]   xform_h1_a12     ,
    output reg    [   31 : 0]   xform_h2_a00     ,
    output reg    [   31 : 0]   xform_h2_a01     ,
    output reg    [   31 : 0]   xform_h2_a02     ,
    output reg    [   31 : 0]   xform_h2_a10     ,
    output reg    [   31 : 0]   xform_h2_a11     ,
    output reg    [   31 : 0]   xform_h2_a12     ,
    ...
    localparam GROUP_CTRL_ADDR   = 16'h29;
    localparam ERROR_STATUS_ADDR = 16'h2A;
    localparam PERF_STAGE_SEL_ADDR = 16'h2B;
    localparam PERF_STAGE_CNT_LO_ADDR = 16'h2C;
    localparam PERF_STAGE_CNT_HI_ADDR = 16'h2D;
    localparam VERSION_ADDR      = 16'h30;
    localparam XFORM_H0_A00_ADDR = 16'h40;
    localparam XFORM_H0_A01_ADDR = 16'h41;
    localparam XFORM_H0_A02_ADDR = 16'h42;
    localparam XFORM_H0_A10_ADDR = 16'h43;
    localparam XFORM_H0_A11_ADDR = 16'h44;
    localparam XFORM_H0_A12_ADDR = 16'h45;
    localparam XFORM_H1_A00_ADDR = 16'h46;
    localparam XFORM_H1_A01_ADDR = 16'h47;
    localparam XFORM_H1_A02_ADDR = 16'h48;
    localparam XFORM_H1_A10_ADDR = 16'h49;
    localparam XFORM_H1_A11_ADDR = 16'h4A;
    localparam XFORM_H1_A12_ADDR = 16'h4B;
    localparam XFORM_H2_A00_ADDR = 16'h4C;
    localparam XFORM_H2_A01_ADDR = 16'h4D;
    localparam XFORM_H2_A02_ADDR = 16'h4E;
    localparam XFORM_H2_A10_ADDR = 16'h4F;
    localparam XFORM_H2_A11_ADDR = 16'h50;
    localparam XFORM_H2_A12_ADDR = 16'h51;
    localparam RESET_ADDR        = 16'h77;
    localparam DEBUG_REG_ADDR    = 16'hFF;

    localparam BEV_VERSION       = 32'h2026_0707; 
    ...
    // Legacy single-step modes were removed. Any start request enters the INT8
    // group4 path; comp_mode is kept as 2'b11 for software readback/status.
    always@(posedge clk) begin
        if(rst_n==1'b0) begin
            comp_start     <= #0.1 1'b0;
            comp_mode      <= #0.1 2'b11;
    ...
    always@(posedge clk) begin
        if(rst_n==1'b0) feat3d_wr_size <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==FEAT3D_WR_SIZE_A) feat3d_wr_size <= #0.1 ra_awdata;
    end

    // ===================== BEV/Image Parameters (defaults for M0-R18) =====================
    // Part1 NPU output: [6, 64, 176, 64] = [cameras, img_h, img_w, channels]
    // BEV grid: 200×200×4 voxels, 64 channels per voxel
    // Part3 input: [1, 200, 200, 4, 256] (4 frames × 64ch = 256ch after concat)
    always@(posedge clk) begin
        if(rst_n==1'b0) begin
    ...
    always@(posedge clk) begin
        if(rst_n==1'b0) begin
            img_w    <= #0.1 12'd176;   // 2D feature map width
            img_h    <= #0.1 12'd64;  // 2D feature map height
    ...
    always@(posedge clk) begin
        if(rst_n==1'b0) frame_size <= #0.1 32'd10240000; // INT8 single-frame BEV: 160000 voxels * 64B
        else if(ra_awevent && ra_awaddr==FRAME_SIZE_ADDR) frame_size <= #0.1 ra_awdata;
    end

    // ===================== Group4 INT8 Extension Registers =====================
    always@(posedge clk) begin
        if(rst_n==1'b0) ext_mode <= #0.1 3'd0;
        else if(ra_awevent && ra_awaddr==EXT_MODE_ADDR) ext_mode <= #0.1 ra_awdata[2:0];
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) feat2d_fp32_base <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==FEAT2D_FP32_ADDR) feat2d_fp32_base <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) feat2d_int8_base <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==FEAT2D_INT8_ADDR) feat2d_int8_base <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) concat_out_base <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==CONCAT_OUT_ADDR) concat_out_base <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) group_ctrl <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==GROUP_CTRL_ADDR) group_ctrl <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) begin
            error_status_w1c       <= #0.1 32'd0;
            error_status_w1c_valid <= #0.1 1'b0;
        end
        else begin
            error_status_w1c_valid <= #0.1 1'b0;
            if(ra_awevent && ra_awaddr==ERROR_STATUS_ADDR) begin
                error_status_w1c       <= #0.1 ra_awdata;
                error_status_w1c_valid <= #0.1 1'b1;
            end
        end
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) perf_stage_sel <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==PERF_STAGE_SEL_ADDR) perf_stage_sel <= #0.1 ra_awdata;
    end

    // ===================== Group4 History Transform Matrices =====================
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h0_a00 <= #0.1 32'h0001_0000;
        else if(ra_awevent && ra_awaddr==XFORM_H0_A00_ADDR) xform_h0_a00 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h0_a01 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H0_A01_ADDR) xform_h0_a01 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h0_a02 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H0_A02_ADDR) xform_h0_a02 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h0_a10 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H0_A10_ADDR) xform_h0_a10 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h0_a11 <= #0.1 32'h0001_0000;
        else if(ra_awevent && ra_awaddr==XFORM_H0_A11_ADDR) xform_h0_a11 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h0_a12 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H0_A12_ADDR) xform_h0_a12 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h1_a00 <= #0.1 32'h0001_0000;
        else if(ra_awevent && ra_awaddr==XFORM_H1_A00_ADDR) xform_h1_a00 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h1_a01 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H1_A01_ADDR) xform_h1_a01 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h1_a02 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H1_A02_ADDR) xform_h1_a02 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h1_a10 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H1_A10_ADDR) xform_h1_a10 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h1_a11 <= #0.1 32'h0001_0000;
        else if(ra_awevent && ra_awaddr==XFORM_H1_A11_ADDR) xform_h1_a11 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h1_a12 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H1_A12_ADDR) xform_h1_a12 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h2_a00 <= #0.1 32'h0001_0000;
        else if(ra_awevent && ra_awaddr==XFORM_H2_A00_ADDR) xform_h2_a00 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h2_a01 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H2_A01_ADDR) xform_h2_a01 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h2_a02 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H2_A02_ADDR) xform_h2_a02 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h2_a10 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H2_A10_ADDR) xform_h2_a10 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h2_a11 <= #0.1 32'h0001_0000;
        else if(ra_awevent && ra_awaddr==XFORM_H2_A11_ADDR) xform_h2_a11 <= #0.1 ra_awdata;
    end
    always@(posedge clk) begin
        if(rst_n==1'b0) xform_h2_a12 <= #0.1 32'd0;
        else if(ra_awevent && ra_awaddr==XFORM_H2_A12_ADDR) xform_h2_a12 <= #0.1 ra_awdata;
    end
    ...
    always@(posedge clk) begin
        if(rst_n==1'b0)
            ...
            case(ra_araddr)
                ...
                EXT_MODE_ADDR     : ra_rdata <= #0.1 {29'd0, ext_mode};
                FEAT2D_FP32_ADDR  : ra_rdata <= #0.1 feat2d_fp32_base;
                FEAT2D_INT8_ADDR  : ra_rdata <= #0.1 feat2d_int8_base;
                CONCAT_OUT_ADDR   : ra_rdata <= #0.1 concat_out_base;
                GROUP_STATUS_ADDR : ra_rdata <= #0.1 group_status;
                GROUP_CTRL_ADDR   : ra_rdata <= #0.1 group_ctrl;
                ERROR_STATUS_ADDR : ra_rdata <= #0.1 group_error_status;
                PERF_STAGE_SEL_ADDR: ra_rdata <= #0.1 perf_stage_sel;
                PERF_STAGE_CNT_LO_ADDR: ra_rdata <= #0.1 perf_cnt_lo;
                PERF_STAGE_CNT_HI_ADDR: ra_rdata <= #0.1 perf_cnt_hi;
                VERSION_ADDR      : ra_rdata <= #0.1 BEV_VERSION;
                XFORM_H0_A00_ADDR : ra_rdata <= #0.1 xform_h0_a00;
                XFORM_H0_A01_ADDR : ra_rdata <= #0.1 xform_h0_a01;
                XFORM_H0_A02_ADDR : ra_rdata <= #0.1 xform_h0_a02;
                XFORM_H0_A10_ADDR : ra_rdata <= #0.1 xform_h0_a10;
                XFORM_H0_A11_ADDR : ra_rdata <= #0.1 xform_h0_a11;
                XFORM_H0_A12_ADDR : ra_rdata <= #0.1 xform_h0_a12;
                XFORM_H1_A00_ADDR : ra_rdata <= #0.1 xform_h1_a00;
                XFORM_H1_A01_ADDR : ra_rdata <= #0.1 xform_h1_a01;
                XFORM_H1_A02_ADDR : ra_rdata <= #0.1 xform_h1_a02;
                XFORM_H1_A10_ADDR : ra_rdata <= #0.1 xform_h1_a10;
                XFORM_H1_A11_ADDR : ra_rdata <= #0.1 xform_h1_a11;
                XFORM_H1_A12_ADDR : ra_rdata <= #0.1 xform_h1_a12;
                XFORM_H2_A00_ADDR : ra_rdata <= #0.1 xform_h2_a00;
                XFORM_H2_A01_ADDR : ra_rdata <= #0.1 xform_h2_a01;
                XFORM_H2_A02_ADDR : ra_rdata <= #0.1 xform_h2_a02;
                XFORM_H2_A10_ADDR : ra_rdata <= #0.1 xform_h2_a10;
                XFORM_H2_A11_ADDR : ra_rdata <= #0.1 xform_h2_a11;
                XFORM_H2_A12_ADDR : ra_rdata <= #0.1 xform_h2_a12;
