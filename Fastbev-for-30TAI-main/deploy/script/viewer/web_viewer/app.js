...
async function loadLiveFrame() {
  ...
  if (["push_waiting", "carla_waiting"].includes(data.live?.mode)) {
  ...
  const isLivePush = ["push", "carla"].includes(data.live?.mode);
  ...
  const isNewPush = ["push", "carla"].includes(live.mode) && Number.isFinite(pushSequence) &&
...
function drawBev(ctx) {
  ...
  ctx.fillStyle = "#fff";
  ctx.fillRect(centre[0], centre[1], 1, 1);
  for (let radius = 10; radius < 100; radius += 10) {
    const rc = radius / 50.0 / 2.0 * bev.size;
    ctx.beginPath();
  ...
  for (const det of detections) {
    ...
    ctx.strokeStyle = nearestRank
      ...
      : selected
        ...
        : canvasColors.detection;
    ctx.lineWidth = width;
    ctx.globalAlpha = alpha;

    drawPolygon(ctx, det.bev.polygon);
    drawLine(ctx, det.bev.heading[0], det.bev.heading[1]);
    ...
    if (nearestRank) {
      ...
      for (const view of det.camera_views) {
        drawRankBadge(ctx, view.bbox[0] + 11, view.bbox[1] + 11, nearestRank);
      }
    }
  }
  ctx.restore();
...
function drawNearestObjectsPanel(ctx, nearest) {
  ...
  for (let row = 0; row < nearestObjectLimit; row++) {
    ...
    ctx.textAlign = "right";
    ctx.fillText(`${det.distance.toFixed(1)} m`, x + panelWidth - 12, cy);
  }
  ctx.restore();
}
...
function showDetection(det) {
  ...
  const rows = [
    ...
    ["Distance", `${det.distance.toFixed(3)} m`],
...
const initialLoad = liveFromUrl()
  ? loadFrameOptions().then(() => startLiveMode())
