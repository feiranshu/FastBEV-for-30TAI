/*
 * CARLA/BEV1 FP16 Part3 validation service using the legacy FP32 bridge.
 *
 *   six 1600x900 JPEGs -> FP32 BGR NHWC -> Part1 runtime FEAT2D PLDDR
 *   -> legacy single-frame Part2 FP32 BEV PLDDR -> PS readback
 *   -> preserve NZYXC + repeat each 64-channel cell x4 -> full FP16 Part3 session
 *   -> threshold/NMS -> non-blocking USB audio alert -> BEV1 JSON result
 *
 * This executable requires the legacy Part2 bitstream that writes 10,240,000
 * FP32 values (40,960,000 bytes). It is intentionally separate from the
 * production INT8 direct-PLDDR CARLA service.
 */
...
constexpr int kTemporalCopies = 4;
...
Arguments parse_arguments(int argc, char** argv) {
    if (argc < 2) {
        throw std::runtime_error(
            "usage: fastbev_pipeline_carla_fp16 <config.yaml> "
...
bool decoder_input_shape_ok(const TensorType& type) {
    return type->shape.size() == 5 && type->shape[0] == 1 &&
           type->shape[1] == 4 && type->shape[2] == 200 &&
           type->shape[3] == 200 && type->shape[4] == 256;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const int bev3d_float_count = lut_count * channels;
        const int bev3d_bytes = bev3d_float_count * fp32_bytes;
        const int decoder_input_channels = channels * kTemporalCopies;
        const int decoder_input_floats = lut_count * decoder_input_channels;

        std::printf("============================================================\n");
        std::printf(" FastBEV CARLA FP16 Service (legacy FP32 PL-PS-PL path)\n");
        std::printf("============================================================\n");
        ...
        const auto decoder_inputs = decoder_network.inputs();
        if (decoder_inputs.size() != 1 || !decoder_input_shape_ok(decoder_inputs[0].tensorType()) ||
            !decoder_inputs[0].tensorType()->element_dtype.isFP32()) {
            throw std::runtime_error(
                "CARLA FP16 decoder public input must be FP32 [1,4,200,200,256]");
        }
        ...
        Session decoder = initSession(
            decoder_backend, decoder_network, device, ocm_option,
            decoder_info.mmu || mmu_mode, speed_mode, compress_ftmp);
        decoder.apply();
        const Value decoder_input = decoder_inputs[0];
        std::printf("[Init] CARLA FP16 Decoder: full network, public input "
                    "FP32 [1,4,200,200,256]\n");
        ...
        auto bev_memory = fpai_device.defaultMemRegion().malloc(bev3d_bytes, 0, 64);
        ...
        const uint32_t bev_address =
            checked_plddr_u32(bev_memory->begin.addr(), "legacy FP32 BEV output");
        ...
        std::printf("[Init] Part2 ready: version=0x%08X LUT=0x%08X\n",
                    fpga_version, lut_address);
        std::printf("[Init] Legacy Part2 FP32 output: addr=0x%08X bytes=%d\n",
                    bev_address, bev3d_bytes);
        std::printf("[Init] WARNING: use the legacy FP32 Part2 BOOT bitstream; "
                    "the INT8 direct-output bitstream is incompatible\n");

        std::vector<float> input_tensor(FASTBEV_TENSOR_SIZE);
        std::vector<float> bev_raw(bev3d_float_count);
        std::vector<float> decoder_input_data(decoder_input_floats);
        ...
        while (true) {
            ...
            try {
                ...
                while (recv_packet(client, packet)) {
                    ...
                    fpai_device.defaultRegRegion().write(kRegFeat3dWrite, bev_address, false);
                    fpai_device.defaultRegRegion().write(kRegFeat3dSize, bev3d_bytes, false);
                    ...
                    const auto bridge_begin = Clock::now();
                    bev_memory.read(reinterpret_cast<char*>(bev_raw.data()), 0, bev3d_bytes);

                    // The new CARLA Part3 calibration contract is NZYXC. Part2
                    // already emits ZYXC/64, so preserve every spatial index and
                    // only repeat its 64 channels into the expected 256 channels.
                    const size_t spatial_cells =
                        static_cast<size_t>(bev_z) * bev_y * bev_x;
                    for (size_t cell = 0; cell < spatial_cells; ++cell) {
                        const float* source = bev_raw.data() + cell * channels;
                        float* destination =
                            decoder_input_data.data() + cell * decoder_input_channels;
                        for (int copy = 0; copy < kTemporalCopies; ++copy) {
                            std::memcpy(
                                destination + static_cast<size_t>(copy) * channels,
                                source,
                                static_cast<size_t>(channels) * sizeof(float));
                        }
                    }
                    timing.bridge_ms = elapsed_ms(bridge_begin, Clock::now());
                    device.reset(1);

                    const auto decoder_input_begin = Clock::now();
                    Tensor decoder_tensor =
                        data2Tensor<float>(decoder_input_data.data(), decoder_input);
                    dmaInit(decoder_backend, decoder_info.ImageMake_on, decoder_tensor, device);
                    timing.decoder_input_ms = elapsed_ms(decoder_input_begin, Clock::now());
                    ...
                    std::printf(
                        ...
                        "pl-ps=%.2f dec_input=%.2f p3_fp16=%.2f post=%.2f\n",
                        ...
                        timing.bridge_ms, timing.decoder_input_ms, timing.part3_ms,
