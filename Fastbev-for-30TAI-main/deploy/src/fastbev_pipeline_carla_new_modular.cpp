...
Arguments parse_arguments(int argc, char** argv) {
    if (argc < 2) {
        throw std::runtime_error(
            ...
            "[--host <address>] [--port <port>] [--control-port <port>] [--source <name>]");
    }
    Arguments args;
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--host" && i + 1 < argc) {
            ...
            args.control_port = std::stoi(argv[++i]);
        } else if (arg == "--source" && i + 1 < argc) {
            args.source = argv[++i];
        } else if (arg == "--help" || arg == "-h") {
            std::printf("usage: %s <config.yaml> [--host <address>] "
                        "[--port <port>] [--control-port <port>] [--source <name>]\n", argv[0]);
            std::exit(0);
        } else {
...
CameraBatch parse_modular_batch(const std::vector<uint8_t>& packet) {
    const auto wire = advwire::parse_perception_batch(packet, kCameraOrder);
    CameraBatch batch;
    batch.frame_id = wire.frame_id;
    batch.capture_ts_ns = wire.capture_ts_ns;
    batch.jpeg = wire.jpeg;
    batch.drive = autodrive::parse_input_json(wire.metadata_json);
    return batch;
}

std::vector<autodrive::Detection> planner_detections(const std::vector<BoundingBox>& boxes) {
    std::vector<autodrive::Detection> out;
    out.reserve(boxes.size());
    for (const auto& box : boxes) {
        autodrive::Detection d;
        d.x=box.x; d.y=box.y; d.z=box.z; d.dx=box.w; d.dy=box.l; d.dz=box.h;
        d.class_id=box.id; d.score=box.score;
        out.push_back(d);
    }
    return out;
}

std::vector<uint8_t> build_modular_result_packet(
    uint64_t frame_id, uint64_t capture_ts_ns, uint64_t finished_ts_ns,
    double inference_ms, double autodrive_ms, const std::string& source,
    const std::vector<BoundingBox>& boxes, const autodrive::Result& drive) {
    std::ostringstream json;
    json.imbue(std::locale::classic());
    json << std::setprecision(9)
         << "{\"frame_id\":" << frame_id
    ...
    for (size_t i = 0; i < boxes.size(); ++i) {
        const auto& box = boxes[i];
        if (i) json << ',';
        json << "{\"x\":" << box.x << ",\"y\":" << box.y << ",\"z\":" << box.z
             << ",\"dx\":" << box.w << ",\"dy\":" << box.l << ",\"dz\":" << box.h
             << ",\"yaw\":" << box.yaw << ",\"class_id\":" << box.id
             << ",\"score\":" << box.score << '}';
    }
    json << "],\"autodrive\":" << autodrive::result_json(drive) << "}";
    return advwire::pack_json(
        advwire::kMessagePerceptionResult, frame_id, finished_ts_ns,
        static_cast<uint32_t>(boxes.size()), json.str());
}

void run_high_rate_control_service(
    const std::string& host, int port, const autodrive::Config& config,
    const std::string& source) {
    int listener = create_listener(host, port);
    std::printf("[ControlListen] %s:%d\n", host.c_str(), port);
    std::fflush(stdout);
    autodrive::Controller controller(config);
    while (true) {
        ...
        // A new CARLA/control session may restart from the beginning while
        // reusing the same route_id. Reset only the high-rate tracker state.
        controller.reset_high_rate_state();
        std::printf("[ControlClient] %s connected\n", peer.c_str());
        try {
            std::vector<uint8_t> packet;
            while (recv_packet(client, packet)) {
                const auto begin = Clock::now();
                const auto message = advwire::parse_json_message(
                    packet, advwire::kMessageControlInput);
                const autodrive::Input input = autodrive::parse_input_json(message.json);
                // High-rate control tracks the received Reference Path directly.
                // Stabilization is implemented inside step_reference_path():
                // monotonic path progress + arc-length lookahead + steer LPF/rate limit
                // + curve-aware speed reduction. It never invokes FOT-Lite.
                const autodrive::Result drive = controller.step_reference_path(input);
                const double processing_ms = elapsed_ms(begin, Clock::now());
                ...
                json << std::setprecision(9)
                     ...
                     << ",\"source\":\"" << json_escape(source + "-high") << "\""
                     << ",\"autodrive\":" << autodrive::result_json(drive) << "}";
                send_packet(client, advwire::pack_json(
                    advwire::kMessageControlResult, message.header.frame_id,
                    finished_ts_ns, 0, json.str()));
            }
        } catch (const std::exception& error) {
            std::fprintf(stderr, "[ControlClient] %s error: %s\n", peer.c_str(), error.what());
        }
        close(client);
    }
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const autodrive::Config autodrive_config = autodrive::load_config(config["autodrive"]);
        autodrive::Controller low_controller(autodrive_config);
        ...
        std::thread control_thread([&args, &autodrive_config]() {
            try {
                run_high_rate_control_service(
                    args.host, args.control_port, autodrive_config, args.source);
            } catch (const std::exception& error) {
                std::fprintf(stderr, "[ControlFatal] %s\n", error.what());
            }
        });
        control_thread.detach();
        std::printf("[Listen] perception %s:%d; high-control %s:%d\n",
                    args.host.c_str(), args.port, args.host.c_str(), args.control_port);
        std::fflush(stdout);
        ...
        while (true) {
            ...
            try {
                std::vector<uint8_t> packet;
                while (recv_packet(client, packet)) {
                    const CameraBatch batch = parse_modular_batch(packet);
                    ...
                    const auto autodrive_begin = Clock::now();
                    const std::vector<autodrive::Detection> drive_detections =
                        planner_detections(valid_boxes);
                    const autodrive::Result drive =
                        low_controller.step_with_fot(batch.drive, drive_detections);
                    const double autodrive_ms = elapsed_ms(autodrive_begin, Clock::now());
                    ...
                    const uint64_t finished_ts_ns = unix_time_ns();
                    const std::vector<uint8_t> result_packet = build_modular_result_packet(
                        batch.frame_id, batch.capture_ts_ns, finished_ts_ns,
                        timing.total_ms, autodrive_ms, args.source, valid_boxes, drive);
