/*
 * Persistent CARLA/BEV1 service for the stable, non-SA FastBEV path.
 *
 *   six 1600x900 JPEGs -> FP32 BGR NHWC -> Part1 runtime FEAT2D PLDDR
 *   -> single-frame Part2 -> CARLA decoder Conv input v251 PLDDR
 *   -> decoder view(15) -> threshold/NMS -> BEV1 JSON result
 *
 * Models, sessions, LUT and PLDDR bindings are initialized once. A malformed
 * client packet closes only that TCP connection; the service then accepts the
 * next Windows gateway connection without reloading the models.
 */
...
// CARLA Part3 ZG contract. Part2 implements the matching 0.07155427
// quantization scale and writes the packed Conv input directly to PLDDR.
constexpr int64_t kDecoderViewStart = 15;
constexpr int64_t kDecoderInputValueId = 251;
constexpr float kDecoderInputBaseScale = 0.07155427f;
...
Arguments parse_arguments(int argc, char** argv) {
    if (argc < 2) {
        throw std::runtime_error(
            "usage: fastbev_pipeline_carla <config.yaml> "
...
bool decoder_input_shape_ok(const TensorType& type) {
    return type->shape.size() == 5 && type->shape[0] == 1 &&
           type->shape[1] == 32 && type->shape[2] == 200 &&
           type->shape[3] == 200 && type->shape[4] == 32;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const int decoder_int8_bytes = lut_count * channels * 4;

        std::printf("============================================================\n");
        std::printf(" FastBEV CARLA Service (BEV1, stable non-SA path)\n");
        std::printf("============================================================\n");
        ...
        if (decoder_inputs.size() != 1 || !decoder_input_shape_ok(decoder_inputs[0].tensorType()) ||
            !decoder_inputs[0].tensorType()->element_dtype.getStorageType().isSInt(8)) {
            throw std::runtime_error("CARLA decoder view(15) input must be sint8 [1,32,200,200,32]");
        }
        ...
        if (decoder_input_id != kDecoderInputValueId) {
            std::ostringstream error;
            error << "CARLA decoder view(" << kDecoderViewStart << ") first input is v"
        ...
        const auto memchunk = fpai_backend->forward_info->memchunk_map.at(decoder_input_id)->memChunk;
        ...
        if (decoder_input.phy_addr < memchunk->begin.addr()) {
            throw std::runtime_error("decoder v251 address is below its MemChunk base");
        }
        decoder_input.offset = decoder_input.phy_addr - memchunk->begin.addr();
        decoder_input.bytes = decoder_inputs[0].tensorType().numElements();
        if (decoder_input.bytes != static_cast<uint64_t>(decoder_int8_bytes) ||
            decoder_input.offset + decoder_input.bytes > memchunk->byte_size) {
            throw std::runtime_error("decoder v251 does not fit in its MemChunk");
        }
        const uint32_t decoder_input_address =
            checked_plddr_u32(decoder_input.phy_addr, "decoder v251");
        std::printf("[Init] CARLA Decoder contract: view_start=%lld v_id=%lld "
                    "shape=[1,32,200,200,32] sint8 base_scale=%.8f\n",
                    static_cast<long long>(kDecoderViewStart),
                    static_cast<long long>(decoder_input_id),
                    static_cast<double>(kDecoderInputBaseScale));
        std::printf("[Init] Decoder Conv input PLDDR: addr=0x%08X bytes=%d\n",
                    decoder_input_address, decoder_int8_bytes);
        ...
        while (true) {
            ...
            try {
                ...
                while (recv_packet(client, packet)) {
                    ...
                    fpai_device.defaultRegRegion().write(
                        kRegFeat3dSize, decoder_int8_bytes, false);
                    ...
                    std::printf(
                        ...
                        "decode+pre=%.2f input=%.2f p1=%.2f p2=%.2f p3=%.2f post=%.2f\n",
